...
# http://blogs.gnome.org/gpoo/2008/04/01/how-many-recent-files-does-an-application-really-need/
...
_REMOVEALL = [ "Image Viewer", "Eye of GNOME Image Viewer", "Reproductor de vídeo Totem" ]
...
def remove_all(manager, all_files):
    ''' Proceed to delete the very recent files.
        We avoid trying to delete a item twice, because the
        second time it will not exists in the manager.
    '''
    ...
    for (app, data) in all_files.iteritems():
        print "app:", app
        if app in _REMOVEALL:
            for uri, visited in data:
                if not deleted.has_key(uri):
                    print "Eliminant", uri
                    try:
                        manager.remove_item(uri)
                    except:
...
if __name__ == '__main__':
    ...
    option_parser.add_option('-a', '--remove-all',
            action='store_true', dest='remove_all', default=False,
            help='Remove those files from selected applications.')
